...
class DownloadFilesTests(unittest.TestCase):
    ...
    async def call_asyncio(self, session, dir, resp_img):
        await self.init_genie.save_images(session, dir, resp_img)

    @patch('download_files.retry_connection.retry')
    def test_status_200_calling_save_images_method(self, MockRetry):
        ...
        self.loop.run_until_complete(self.call_asyncio(session, dir, resp_img))
        path = dir +"/"+f"{resp_img.rsplit('/', 1)[-1]}.png"
        self.assertTrue(os.path.exists(path))

    @patch('download_files.retry_connection.retry')
    def test_status_403_and_exception_when_file_name_is_worng_calling_save_images_method(self, MockRetry):
